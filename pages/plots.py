...
def toggle_chart_in_pdf(n_clicks_list, pdf_list, current_viz):
    # Si no hay trigger o aún no hay gráfico generado, no hacemos nada
    if not ctx.triggered or current_viz is None:
        raise PreventUpdate

    # Aseguramos que pdf_list sea una lista
    if pdf_list is None:
        pdf_list = []

    # ID del botón que se clicó: {"type": "btn-pdf-toggle", "index": "airflow_raw", ...}
    trigger_id = ctx.triggered_id
    chart_id = trigger_id["index"]

    # ¿Ya está este gráfico en la lista?
    ...
    if is_in_list:
        ...
        pdf_list.append(
            {
                "id": chart_id,
                "html_path": current_viz.get("html_path"),
                "title": current_viz.get("title"),
            }
        )

    # Actualizar estado del botón "Generate PDF" y contador
...
def generate_pdf_report(n_clicks, pdf_list):
    ...
    try:
        ...
        # Devolver el PDF para que dcc.Download lo descargue
